...
BEGIN
	...
	DECLARE @OpeningBalance decimal(18,4) = 0
	   ...
	   AND  cbt.TransactionDate >= @FromDate AND  cbt.TransactionDate <= @ToDate
	   ORDER BY (case when cbt.CategoryType ='Inter' then 1 when cbt.CategoryType ='Vendor' then 2 when cbt.CategoryType ='Client' then 3 when cbt.CategoryType ='Consultant' then 4 when cbt.CategoryType ='Auto-Debit' then 5 else 6 end)  Desc
END
GO
