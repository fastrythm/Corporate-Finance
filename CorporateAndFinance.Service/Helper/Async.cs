...
namespace CorporateAndFinance.Service.Helper

{
    public static class Async {
       // private static ILog logger = LogManager.GetLogger(typeof (Async));
...
//logger.Error("Failed to queue in threadpool, Method: " + key);
        ...
        private static void FinishInvoke(AsyncRes res) {
            ...
            try {
             ...
             //   logger.Error("Error setting wait handle on " + (res.Method ?? "NULL"), ex);
            }
            ...
            if (res.RMode != ReenteranceMode.Allow) {
                ...
                try {
                   ...
                   // logger.Error("Error releasing reentrant lock on " + (res.Method ?? "NULL"), ex);
                }
            }
        }
